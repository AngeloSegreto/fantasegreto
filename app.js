...
async function fetchData(){
 const [r,d,o,v,pv3,mv3,cand,sl63]=await Promise.all([
 ...
 for(const p of merged.players){if(pv3.player_performance_records?.[p.id])p._performanceV3={...pv3.player_performance_records[p.id]};if(mv3.records?.[p.id])p._medicalV3={...mv3.records[p.id]};p._evidence23=cMap.get(p.id)||'PENDING_EVIDENCE';if(slMap.has(p.id))p._shortlist63=slMap.get(p.id)}
 S.registry=merged.players;S.details=merged.details;S.news=o;S.shortlist63=sl63;
 window.FS_PERFORMANCE_V3=pv3;window.FS_MEDICAL_V3=mv3;window.FS_FOS_CANDIDATES=cand;window.FS_SHORTLIST63=sl63;
...
function runtimeLabel(p,st){const lm=st?dynamicMax(p,st):0,med=p._medical22?.badge||p._medical22?.status;let label;if(!st)label=p.decision||'—';else if(lm<=0)label=p.serie_a_membership==='ACTIVE_CURRENT_LISTONE_22SEP'?'SERIE A · RECALC':'PASS · RECALC';else{const ideal=Number(p.ideal||0);label=lm<=ideal?`BUY/WAIT ≤ ${lm}`:`BUY ≤ ${ideal} · WAIT ≤ ${lm}`}const stale=p.model_status==='STALE_PRE_22SEP_RECALC';if(stale)label+=` · PRE22`;const mq=st?window.FS_MARKET_ENGINE?.quote?.(p,st):null;if(mq)label+=` · MKT ${mq.expected}`;return med?`${label} · ${med}`:label}
function renderList(){const sold=soldIds(),avail=S.filtered.filter(p=>!sold.has(p.id)),a=avail.slice(0,S.shown);$('#count').textContent=`${avail.length} disponibili · ${a.length} renderizzati`;const st=state();$('#list').innerHTML=a.map(p=>{const live=runtimeLabel(p,st),sl=p._shortlist63,label=S.targetOnly&&sl?`T0 BUY ≤ ${sl.buy} · WAIT ${sl.wait[0]}–${sl.wait[1]} · STOP > ${sl.stop}`:live,k=(S.targetOnly&&sl)?'BUY':label.startsWith('BUY')?'BUY':label.startsWith('PASS')?'PASS':'WAIT',lm=st?dynamicMax(p,st):0,score=num(p.score);return `<article class="row" data-id="${esc(p.id)}"><span class="role ${esc(p.role)}">${esc(p.role)}</span><div class="name"><b>${esc(p.name)}</b><small>${esc(p.team)} · ${sl?'TARGET 63 · MKT T0 '+esc(sl.market_t0):esc((p.list_meta||'').replace(p.team+' · ','')||p.role)}</small><span class="decision ${k}">${esc(label)}</span></div><div class="nums"><b>${score===null?'—':esc(score.toFixed(1))}</b><small>${lm>0?'MAX LIVE '+esc(lm):'MAX RECALC'}</small></div></article>`}).join('')||'<div class="empty">Nessun giocatore disponibile.</div>';$('#more').hidden=a.length>=avail.length}
...
function open(id){const p=S.registry.find(x=>x.id===id),d=S.details[id];if(!p||!d)return;S.selected=p;const st=state();$('#s-role').className='role '+p.role;$('#s-role').textContent=p.role;$('#s-name').textContent=p.name;$('#s-sub').textContent=d.subtitle||`${p.team} · ${p.role}`;const lm=st?dynamicMax(p,st):0,metrics={'FS Score':num(p.score)===null?'RECALC':Number(p.score).toFixed(1),'Prezzo ideale':num(p.ideal)===null?'RECALC':p.ideal,'MAX base':num(p.max)===null?'RECALC':p.max,'Quotazione':num(p.quote)===null?'—':p.quote,'FVM':num(p.fvm)===null?'—':p.fvm,'Rischio':num(p.risk)===null?'—':p.risk,'Confidence':p.confidence||'—'};metrics['Serie A 22/09']=p.serie_a_membership==='ACTIVE_CURRENT_LISTONE_22SEP'?'PRESENTE':'VERIFICA';metrics['Model status']=p.model_status||'—';metrics['Kapitals 22/09']=p.kapitals_22sep??'—';if(p._performance22?.appearances!=null)metrics['Presenze report']=p._performance22.appearances;if(p._performance22?.goals!=null)metrics['Gol 22/09']=p._performance22.goals;if(p._performance22?.assists!=null)metrics['Assist 22/09']=p._performance22.assists;if(p._performance22?.chances_created!=null)metrics['Occasioni create']=p._performance22.chances_created;if(p._team22)metrics['Team 5G']=`${p._team22.pts} pt · GF ${p._team22.gf} · GS ${p._team22.ga}`;if(p._medical22)metrics['Medical 22/09']=p._medical22.source_status;if(p._shortlist63){metrics['Target 63']='SÌ';metrics['BUY T0']='≤ '+p._shortlist63.buy;metrics['WAIT T0']=p._shortlist63.wait[0]+'–'+p._shortlist63.wait[1];metrics['STOP T0']='> '+p._shortlist63.stop;metrics['MKT shortlist']=p._shortlist63.market_t0}if(st){metrics['MAX ASTA']=lm>0?lm:'RECALC';const mq=window.FS_MARKET_ENGINE?.quote?.(p,st);if(mq){metrics['Mercato T0']=mq.market_t0;metrics['Clearing atteso']=mq.expected;metrics['Tier mercato']=mq.tier;metrics['Gap mercato/MAX']=mq.market_gap}const op=window.FS_AUCTION_ENGINE?.opponentPressure?.(p,st);if(op){metrics['Pressione core']=op.pressure+'/100';metrics['Rivali attivi']=op.activeCount+'/9'}}$('#metrics').innerHTML=Object.entries(metrics).map(([k,v])=>`<div class="metric"><small>${esc(k)}</small><b>${esc(v)}</b></div>`).join('');const inst=window.FS_AUCTION_ENGINE?.instruction?.(p,st)||(d.instruction||p.decision);$('#instruction').innerHTML=`<b>Istruzione d’asta LIVE</b><p>${esc(inst)}</p>`;const extraBlocks=[...(d.blocks||[])];if(p._shortlist63?.substitutes?.length)extraBlocks.unshift({title:'Pivot Target 63',text:p._shortlist63.substitutes.map((x,i)=>`${i+1}. ${x.name} (${x.team}) · BUY ≤ ${x.buy} · WAIT ${x.wait[0]}–${x.wait[1]} · STOP > ${x.stop}`).join(' | ')});$('#blocks').innerHTML=extraBlocks.map(x=>`<div class="block"><b>${esc(x.title)}</b><p>${esc(x.text)}</p></div>`).join('');renderPressure(p,st);$('#price').value='';renderOwnerOptions();$('#owner').value='';$('#runtime').textContent='Engine V4.0.23.1 LOCK · MARKET V1 · 10×500 · '+(lm>0?'DYNAMIC MAX '+lm:'MAX RECALC');$('#sheet').classList.add('open');document.body.style.overflow='hidden';setTimeout(()=>$('#price').focus({preventScroll:true}),80)}
